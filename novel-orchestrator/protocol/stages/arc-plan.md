# stage:arc — 弧规划（含 traditional 细纲工序）配套知识包

| 键 | 值 |
|---|---|
| 阶段 | arc：弧六节规划（court.md §1 弧简流程行）；traditional 另含细纲工序（route-traditional §2） |
| 进入 | 写作游标距弧前沿 ≤1 弧（gate next 会提示） |
| 退出 | arc_NN_n 六节 committed + 简式 dec；traditional 另加细纲批过 structure-critic 检查 |
| 下一阶段 | write（排批进产线） |
| 本包读者 | 编排者（装配弧简流程投递时） |
...
| K-ID | 名称 | 何时挑 |
|---|---|---|
| K-STRUCT-014 | 危机设计 | 弧高潮前铺排 |
| K-WRITE-010 | 步骤大纲法 | traditional 细纲工序参考 |
| K-WRITE-013 | 读者兴趣三策略 | 期待操作表设计 |
