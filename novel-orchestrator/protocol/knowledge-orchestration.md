...
| L-diag 诊断层 | diag 默认路由，或任一已进入阶段的有界咨询 | diagnose.md **包内**症状路由（diag 专属 33 块池）→ K-ID → knowledge-blocks 锚点 | ≤2 块/次，读完即弃 |

## 3. 禁装规则（forbid；违者按泄漏处理，pipeline §5）

1. 不把知识原文整库投给 worker。具体问题可以在设计、写作或评审时咨询；编排者将来源、适用步骤与正反例编入任务或设计简报。知识维护归属不是使用权限，写手不自行扩展事实边界。
