...
| 章尾钩 | 诊断：可承接全局期待或完整收束 | 同左，非强制悬崖钩 |
| 爽点窗口 | 三章/十章仅作统计观察 | 同左，按本书体验判断 |
...
3. `hook.close` 可以为 null；`turn` 可说明本章的事件、关系、理解或体验作用，不强制正负翻转；
...
| 断言 | web | traditional |
|---|---|---|
| `check --unit` hooks_realized.close=false | WARN，交编辑判断 | WARN，交编辑判断 |
| `check --window` 三章/十章样例窗口 | WARN | WARN |
